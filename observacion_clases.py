...
# --------------------------------------------------
# Nota: mantenemos el esquema original basado en posiciones (M–AZ),
# pero podría moverse a una configuración por nombres si el formulario cambia.
...
with tab_resumen:
    ...
    # Para ver la evolución, usamos los datos filtrados por servicio/tipo,
    # pero sin limitar a un solo corte.
    ...
    # Agrupar por corte y clasificación
...
with tab_docentes:
    ...
    # Ordenar mostrando primero consolidado, luego en proceso, luego no consolidado
    ...
    resumen_docente = resumen_docente.sort_values(
        ["Clasificación_docente", "Promedio_puntos_por_obs"], ascending=[True, False]
    )
...
with tab_detalle:
    ...
    # Lista de docentes disponibles en el filtro actual
    resumen_docente = (
        df_filtrado.groupby(COL_DOCENTE)
        .agg(
            N_observaciones=("Total_puntos_observación", "count"),
    ...
    if docente_sel != "(ninguno)":
        ...
        # Historial general del docente en el filtro
        cols_hist = [
            col_fecha,
            COL_SERVICIO,
            "Grupo",
        ...
        # -------------------------
        # Resumen por áreas (promedio del docente en el filtro)
        # -------------------------

        def calcular_resumen_areas(df, columnas_area):
            """Calcula puntos totales, máximo posible y % logro para un conjunto de columnas."""
            puntos_totales = 0
            max_puntos = 0
